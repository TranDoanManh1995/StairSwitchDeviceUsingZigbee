 ...
 * @param handleNumberPressesCallback - The callback function will be called to process the number of button presses
 * @retval None
 */
void BUTTON_initializeButton(BUTTON_handleNumberButtonPresses_t handleNumberButtonPressesCallback)
{
	...
	GPIOINT_Init();
		...
		// Configure GPIO mode & interrupt
	for (uint8_t i = 0; i < NUMBER_BUTTON; i++){
		GPIO_PinModeSet(informationButton[i].GPIOportButton,
						informationButton[i].GPIOpinButton,
						gpioModeInputPullFilter,
						BUTTON_PULLUP_FILTER_INPUT);
		GPIOINT_CallbackRegister(informationButton[i].GPIOpinButton, BUTTON_handleInterruptButton);
		GPIO_ExtIntConfig(informationButton[i].GPIOportButton,
						  informationButton[i].GPIOpinButton,
						  informationButton[i].GPIOpinButton,
						  true,
						  true,
